...
// Enhanced month format conversion
...
// Helper function to check for MRN duplicates within the same month
...
async function initializeDatabase() {
    try {
        ...
        // Create patients table with proper MRN constraints
        await safeQuery(`
        ...
        // Remove old constraints and add proper MRN constraint
        await safeQuery(`
...
// FIXED: SIMPLIFIED CSV EXPORT ENDPOINT
app.get('/api/export/supply-report', authenticateToken, async (req, res) => {
    try {
        ...
        console.log('Executing export query with', params.length, 'parameters');
        const result = await safeQuery(query, params);
        console.log('Export query returned', result.rows.length, 'rows');
        ...
        // CSV escaping function with MRN formatting
        const escapeCsv = (val, isMRN = false) => {
            if (val === null || val === undefined) return '';
            
            let strVal = String(val);
            
            // Special MRN formatting to prevent scientific notation in Excel
            if (isMRN && strVal && strVal.trim() !== '' && strVal.trim() !== 'N/A') {
                strVal = "'" + strVal.trim(); // Leading apostrophe forces text format
            }
        ...
        result.rows.forEach(row => {
            ...
            const csvRow = [
                ...
                escapeCsv(row.mrn || 'N/A', true), // TRUE for MRN formatting
                escapeCsv(row.month),
